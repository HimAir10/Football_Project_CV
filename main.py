...
def get_kits_classifier(kits_colors):
    ...
    """
    if not kits_colors or len(kits_colors) < 2:
        raise ValueError(f"Insufficient kit colors for clustering. Received: {kits_colors}")
...
def annotate_video(video_path, model):
    ...
    output_path = os.path.join(output_dir, video_name.split('.')[0] + "_out.mp4")
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            if current_frame_idx == 1:
                if len(players_imgs) >= 2 and len(kits_colors) >= 2:
                    ...
                    print(f"Frame {current_frame_idx}: Not enough players or kit colors to build classifier.")
                    # Skip processing boxes if classifier isn't ready
                    output_video.write(annotated_frame)
                    continue

            # Only process boxes if classifier is ready
            if kits_clf is not None:
                for box in result.boxes:
                    ...
                    if label < len(labels):
                        ...
                        print(f"⚠️ Label {label} is out of range. Max available: {len(labels) - 1}")
            else:
                # If classifier not ready, just draw basic boxes without team classification
                for box in result.boxes:
                    label = int(box.cls.cpu().numpy()[0])
                    x1, y1, x2, y2 = map(int, box.xyxy[0].cpu().numpy())

                    # Use original labels for non-player/GK objects, or generic labels for players
                    if label == 0:
                        ...
                        display_label = "GK"
                        color = (128, 128, 128)
                    elif label < len(labels) - 2:
                        display_label = labels[label + 2]
                        color = box_colors.get(str(label + 2), (255, 255, 255))
                    else:
                        display_label = f"Class_{label}"
                        color = (255, 255, 255)

                    cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 2)
                    cv2.putText(annotated_frame, display_label, (x1 - 30, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                                color, 2)
...
if __name__ == "__main__":
    ...
    labels = [
        "Player-L",  # 0 - Left team player
        "Player-R",  # 1 - Right team player
        "GK-L",  # 2 - Left team goalkeeper
        "GK-R",  # 3 - Right team goalkeeper
        "Ball",  # 4 - Ball (original label 2 + 2)
        "Main Ref",  # 5 - Main Referee (original label 3 + 2)
        "Side Ref",  # 6 - Side Referee (original label 4 + 2)
        "Staff"  # 7 - Staff (original label 5 + 2)
        # 13 - Maximum possible (original label 11 + 2)
    ]
    ...
    model = YOLO("/content/runs/detect/finetune_v23/weights/best.pt")
    video_path = "/content/TeamsRefGKBall-14/15sec_input_720p (1)_out.mp4"
